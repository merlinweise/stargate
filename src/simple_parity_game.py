...
class SPG_Transition:
    def __init__(self, start_vertex: SPG_Vertex, end_vertices: set([(float, SPG_Vertex)]), action: str):
        """
...
class Simple_Parity_Game:

    def __init__(self, vertices: dict([(str, SPG_Vertex)]), transitions: dict([((SPG_Vertex, str),SPG_Transition)]), init_vertex):
        """
        ...
        for vertex in self.vertices.values():
            ...
            if is_deadlock_vertex(vertex, self.transitions):
                ...
                self.transitions[vertex, "selfloop"] = SPG_Transition(vertex, {(1.0, vertex)}, "selfloop")

def has_ingoing_transition(vertex: SPG_Vertex, transitions: dict([((SPG_Vertex, str),SPG_Transition)])) -> bool:
    """
...
def is_deadlock_vertex(vertex: SPG_Vertex, transitions: dict([((SPG_Vertex, str),SPG_Transition)])) -> bool:
    """
...
def read_spg_from_file(file_name: str) -> Simple_Parity_Game:
    if file_name[-4:] != ".spg":
        ...
        match state:
            case 0:
                ...
                if current_line == ["evevertices"]:
                    state=1
                    continue
                print_error(f"File does not comply with spg specification. Line {current_line_index+1}: {content[current_line_index]}")
            ...
            case 1:
                ...
                if current_line == ["endevevertices"]:
                    state=2
                    continue
                if current_line[1] == ":" and current_line[2].isdigit() and int(current_line[2])>=0 and len(current_line) == 3:
                    if current_line[0] in spg_vertices:
                        print_error(f"Duplicate vertex {current_line[0]}")
                    spg_vertices[current_line[0]] = SPG_Vertex(current_line[0], True, int(current_line[2]))
            ...
            case 3:
                ...
                if current_line[1] == ":" and current_line[2].isdigit() and int(current_line[2])>=0 and len(current_line) == 3:
                    if current_line[0] in spg_vertices:
                        print_error(f"Duplicate vertex {current_line[0]}")
                    spg_vertices[current_line[0]] = SPG_Vertex(current_line[0], False, int(current_line[2]))
                    # print(spg_vertices[current_line[0]])
                    continue
                print_error(f"File does not comply with spg specification. Line {current_line_index + 1}: {content[current_line_index]}")
            ...
            case 6:
                ...
                if len(current_line) == 4 and current_line[2] == ":":
                    ...
                    if (spg_vertices[current_line[0]], current_line[1]) in spg_transitions:
                        print_error(f"Duplicate transition from {spg_vertices[current_line[0]]} with action {current_line[1]}")
                    spg_transitions[(spg_vertices[current_line[0]], current_line[1])] = SPG_Transition(spg_vertices[current_line[0]], {(1.0, spg_vertices[current_line[3]])}, current_line[1])
                ...
                if len(current_line) >= 6 and len(current_line) % 4 == 2 :
                    ...
                    spg_transitions[(spg_vertices[current_line[0]], current_line[1])] = SPG_Transition(spg_vertices[current_line[0]], end_vertices, current_line[1])
                    continue
                print_error(f"File does not comply with spg specification. Line {current_line_index + 1}: {content[current_line_index]}")
            case _:
                print_error("Undefined state")
                sys.exit(1)
    return Simple_Parity_Game(spg_vertices, spg_transitions, spg_initial_vertex)

def spg_to_spgspec(spg: Simple_Parity_Game, file_name: str="", force: bool=False):
    content = "spg\n\n"
